...
4. Plot the Measles disease rates per year for California. Use the the square root transformation of the y-axis. Make sure that the numbers $4,16,36, \dots, 100$ appear on the y-axis. Find out when the Measles vaccine was introduced and add a vertical line to the plot to show this year:
...
Now, to make the plot we simply use the `geom_line` geometry, and add the vertical line. 
...
**Solution:** After making the plot below once we notice that past 1980 counts were often not reported. So we show data up until 1980. Students don't have to do this and don't take off points. 
...
```{r}
autism_prevalence_CA %>% ggplot(aes(x = year, y = prevalence)) +
	geom_point() +
	geom_line() +
	geom_vline(xintercept = 1963, col = "blue") +
	geom_text(aes(x = 1962, y = 0.5, label = "Measles vaccine introduced 1963"), color="blue", angle = 90) +
	geom_vline(xintercept = 1971, col = "red") +
	geom_text(aes(x = 1970, y = 0.5, label = "Mumps/Rubella vaccine introduced 1971"), color="red", angle = 90) +
	geom_vline(xintercept = 1955, col = "darkgreen") +
	geom_text(aes(x = 1954, y = 0.5, label = "Polio vaccine introduced 1955"), color="darkgreen", angle = 90) +
	geom_vline(xintercept = 1914, col = "orange") +
	geom_text(aes(x = 1913, y = 0.5, label = "Pertussis vaccine introduced 1914"), color="orange", angle = 90) +
	geom_vline(xintercept = 1995, col = "purple") +
	geom_text(aes(x = 1994, y = 0.5, label = "Hepatitis A vaccine introduced 1995"), color="purple", angle = 90) +
	xlab("") +
	ylab("Autism prevalence") + 
	ggtitle("CA Autism Prevalence Over Time")
```

While the prevalence of autism has increased over time, as seen in the plot, this increase was not caused by vaccines. The definition of autism has changed and broadened over time, and the public has become more aware of the condition over time, prompting more diagnoses. 
