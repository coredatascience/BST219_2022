...
sim_index_train = createDataPartition(y = sim_dat$Y, 
                                      times = 1, p = 0.5, list = FALSE) # create one data partition, 50-50
...
```{r}
probs_knn1 = predict(fit_knn1, newdata = sim_test_set)[,2] #interested in predicting class 1, that is Y = 1
...
#Another way to extract from a list:
#predict(fit_lda, newdata = sim_test_set)[["posterior"]]
```
...
```{r}
#Another way to plot ROC curves using ggroc -- looks nicer!

ggroc(list("QDA" = roc_qda, "LDA" = roc_lda, "kNN, k = 1" = roc_knn1, "kNN, k = 15" = roc_knn15)) +
...
A promised in lab, here a few packages you can use to make nice tables in R. I think this will be useful for your projects and future analyses in R.
...
#stargazer: really good for regression models. https://cran.r-project.org/web/packages/stargazer/vignettes/stargazer.pdf
#gt: https://gt.rstudio.com/articles/intro-creating-gt-tables.html

#Example using kableExtra
metrics<-data.frame(Model = c("kNN1","kNN15","LDA", "QDA"), AUC = c(roc_knn1$auc, roc_knn15$auc, roc_lda$auc, roc_qda$auc))
metrics %>% kbl() %>% kable_classic_2(full_width = F)


#Example of table 1 -- you can also use make.table() and build your own!
    ##You can change labels or categories of your variables
label(sim_dat$Y)<-"Outcome"
sim_dat$Y<-factor(sim_dat$Y, levels = 0:1, labels = c("Class 0", "Class 1"))

table1(~X1 + X2 + Y, data = sim_dat)
