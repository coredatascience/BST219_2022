...
co2_tidy <- 
  co2_wide %>% 
  pivot_longer(cols = -year, ### Everything (all columns) except the year
               names_to = 'month', ### name of column corresponding to column names from wide format
               values_to = 'co2' ### name of column corresponding to values from wide format
  )

### Can we recover co2_wide?
co2_tidy %>% 
  pivot_wider(id_cols = year, ### what should not be pivoted to wider
              names_from = month,  ### which column do we use to set col_names in wideformat
              values_from = co2 ### where do we take values from 
  )
...
```{r}
ggplot(co2_tidy, aes(x = month, y = co2, col = year)) + 
  geom_line()
```
...
```{r}
### R stores colnames always as chr so have to be careful when we pivot
...
### Why can we avoid using aes(group = year)?
ggplot(co2_tidy, aes(x = month, y = co2)) + 
  geom_line()

ggplot(co2_tidy, aes(x = month, y = co2, group = year)) + 
  geom_line()

### What happens if year is numeric?
ggplot(co2_tidy, aes(x = month, y = co2, col = as.numeric(year))) + 
  geom_line()

### Now we need group!
ggplot(co2_tidy, aes(x = month, y = co2, col = as.numeric(year), group = year)) + 
  geom_line()
...
```{r}
### always remember to select the ID(s) to join on
df_names <- 
  select(People, playerID, nameFirst, nameLast)

top <- 
  top %>% 
  left_join(df_names, by = 'playerID') 
